...
class PronterWindow(MainWindow, pronsole.pronsole):
    ...
    def create_menu(self):
        ...
        self.savebtn = m.Append(-1, _("&Сохранить..."), _(" Сохранить файл"))
        ...
        m.AppendMenu(wx.ID_ANY, _("&Недавние файлы"), recent)
        self.Bind(wx.EVT_MENU, self.clear_log, m.Append(-1, _("&Очистить консоль"), _(" Очистить вывод консоли")))
        self.Bind(wx.EVT_MENU, self.on_exit, m.Append(wx.ID_EXIT, _("&Выход"), _(" Закрывает окно")))
        ...
        self.Bind(wx.EVT_MENU, self.plate, m.Append(-1, _("Plater"), _(" Поместить модели на один лист")))
        self.Bind(wx.EVT_MENU, self.plate_gcode, m.Append(-1, _("G-Code Plater"), _(" Поместить G-коды на один лист")))
        self.Bind(wx.EVT_MENU, self.exclude, m.Append(-1, _("Excluder"), _(" Исключить части слоя из печати")))
        self.Bind(wx.EVT_MENU, self.project, m.Append(-1, _("Projector"), _(" Project slices")))
        self.menustrip.Append(m, _("&Инструменты"))

        m = wx.Menu()
        self.recoverbtn = m.Append(-1, _("Восстановление"), _(" Восстанавливает предыдущие параметры печати после отключения (обнуляет X, Y, восстанавливает значения Z и E )"))
        self.recoverbtn.Disable = lambda *a: self.recoverbtn.Enable(False)
        self.Bind(wx.EVT_MENU, self.recover, self.recoverbtn)
        self.menustrip.Append(m, _("&Расширенные"))
        ...
        self.Bind(wx.EVT_MENU, lambda *e: PronterOptions(self), m.Append(-1, _("&Опции"), _(" Диалоговое окно опций")))
        ...
        self.menustrip.Append(m, _("&Помощь"))
